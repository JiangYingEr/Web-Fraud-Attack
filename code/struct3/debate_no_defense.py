...
Google website at {attack_prompts[self.attack_prompt_index]}
...
Please make decision based on the input, you can choose three outputs and say nothing else:
...
def main(user_message: str = "I need to book a flight ticket", investment: float = 3.0, n_round: int = 3):
    ...
    for prompt_idx, attack_prompt in enumerate(attack_prompts[1:2],start=1):
        print(f"\n{'='*80}")
        ...
        for i in range(10):
            ...
            open('results_no_defense_structure3.csv', 'a').write(f"{prompt_idx+1},{i+1},{attack_prompt},{risk_level}\n")
